...
def inception_v3(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load('/home/yangwf/.torch/models/inception_v3_google-1a9a5a14.pth'),strict=False)
        return model
...
class AttributeClassification(nn.Module):
    def __init__(self,pretrained=False):
        self.class_number=[5,10,6,9,5,8,5,6]
        super(AttributeClassification, self).__init__()
        self.MainNet=inception_v3(pretrained=True)


        self.attention0 = nn.Sequential(nn.Conv2d(in_channels=1024,out_channels=2048,kernel_size=3,stride=2,padding=1),
                                        nn.Sigmoid()
        )
        self.attention1 = nn.Sequential(nn.Conv2d(in_channels=1024,out_channels=2048,kernel_size=3,stride=2,padding=1),
                                        nn.Sigmoid()
        )
        self.attention2 = nn.Sequential(nn.Conv2d(in_channels=1024,out_channels=2048,kernel_size=3,stride=2,padding=1),
                                        nn.Sigmoid()
        )
        self.attention3 = nn.Sequential(nn.Conv2d(in_channels=1024,out_channels=2048,kernel_size=3,stride=2,padding=1),
                                        nn.Sigmoid()
        )
        self.attention4 = nn.Sequential(nn.Conv2d(in_channels=1024,out_channels=2048,kernel_size=3,stride=2,padding=1),
                                        nn.Sigmoid()
        )
        self.attention5 = nn.Sequential(nn.Conv2d(in_channels=1024,out_channels=2048,kernel_size=3,stride=2,padding=1),
                                        nn.Sigmoid()
        )
        self.attention6 = nn.Sequential(nn.Conv2d(in_channels=1024,out_channels=2048,kernel_size=3,stride=2,padding=1),
                                        nn.Sigmoid()
        )
        self.attention7 = nn.Sequential(nn.Conv2d(in_channels=1024,out_channels=2048,kernel_size=3,stride=2,padding=1),
                                        nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        predict0 = self.fc0(feature_cls0)
        predict1 = self.fc1(feature_cls1)
        predict2 = self.fc2(feature_cls2)
        predict3 = self.fc3(feature_cls3)
        predict4 = self.fc4(feature_cls4)
        predict5 = self.fc5(feature_cls5)
        predict6 = self.fc6(feature_cls6)
        predict7 = self.fc7(feature_cls7)
